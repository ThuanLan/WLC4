...
public class ServicesPageObject extends mobioLibs {
    ...
    public void deleteDataToResetFBCommentModule() {
        deleteDataToReset(GlobalConstants.REQUEST_URL_TO_DELETE_DATA, GlobalConstants.MERCHAN_ID,
                GlobalConstants.FB_SOCIAL_TYPE_ID, GlobalConstants.COMMENT_OBJECT_TYPE_ID);
    }

    public void deleteDataToResetFBMessageModule() {
        deleteDataToReset(GlobalConstants.REQUEST_URL_TO_DELETE_DATA, GlobalConstants.MERCHAN_ID,
                GlobalConstants.FB_SOCIAL_TYPE_ID, GlobalConstants.MESSAGE_OBJECT_TYPE_ID);
    }

    public void deleteDataToResetWLCModule() {
        deleteDataToReset(GlobalConstants.REQUEST_URL_TO_DELETE_DATA, GlobalConstants.MERCHAN_ID,
                GlobalConstants.WEBLIVECHAT_SOCIAL_TYPE_ID, GlobalConstants.MESSAGE_OBJECT_TYPE_ID);
    }
    ...
    public void selectOnlyAssignToOnlineMemberRule() {
        ...
        while (isElementDisplayed(driver, ServicesPageUI.ACTIVE_CONFIG_TOGGLE)) {
            ...
            // scrollToElement(driver, ServicesPageUI.ASSIGNMENT_CONFIG_SAVE_BTN);
        ...
        if (isElementDisplayed(driver, ServicesPageUI.SELECTED_TYPE_ASSIGNMENT_LBL)) {
            ...
            selectItemInCustomDropdown(driver, ServicesPageUI.ASSIGNMENT_TYPE_DROP,
                    ServicesPageUI.LIST_ASSIGNMENT_TYPE_DROP, "Phân chia cho tất cả các thành viên trong Team");
            waitToElementClickable(driver, ServicesPageUI.ASSIGNMENT_CONFIG_SAVE_BTN);
            clickToElement(driver, ServicesPageUI.ASSIGNMENT_CONFIG_SAVE_BTN);
            waitShortToElementInVisible(driver, CommonPageUI.LOADING_ICON);
        }
    ...
    public void configRecallRule() {
        if (isElementDisplayed(driver, ServicesPageUI.RECALL_LAW_ACTIVE_TOGGLE)) {
            ...
            selectItemInCustomDropdown(driver, ServicesPageUI.RECALL_ASSIGN_MEMBER_DROP,
                    ServicesPageUI.LIST_MEMBER_DROP, "Admin (admin@testauto)");
            waitToElementClickable(driver, ServicesPageUI.ASSIGNMENT_CONFIG_SAVE_BTN);
            clickToElement(driver, ServicesPageUI.ASSIGNMENT_CONFIG_SAVE_BTN);
        } else {
            clickToElement(driver, ServicesPageUI.RECALL_LAW_TOGGLE);
    ...
    public List<String> getListPageNameOfATeam(String socialNetworkName) {
        ...
        List<WebElement> configuredPageList = findElementsByXpath(driver,
                ServicesPageUI.LIST_SOCIAL_PAGE_NAME_CHECKED_ITEM, socialNetworkName);
